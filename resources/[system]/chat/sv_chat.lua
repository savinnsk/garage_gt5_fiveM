...
AddEventHandler('onServerResourceStart', function(resName)
    ...
    RegisterCommand('evento', function(source, args, rawCommand)
        ...
        local identity = vRP.getUserIdentity(user_id)
        fal = identity.name.. " " .. identity.firstname
        if vRP.hasPermission(user_id, "owner.permissao") or vRP.hasPermission(user_id, "admin.permissao") or vRP.hasPermission(user_id, "moderador.permissao") then
        TriggerClientEvent('chat:addMessage', -1, {
            template = '<div style="padding: 0.5vw; margin: 0.5vw; background-image: linear-gradient(to right, rgba(255, 255, 10,0.7) 3%, rgba(0, 0, 0,0) 95%); border-radius: 15px 50px 30px 5px;"><img style="width: 17px" src="https://as1.ftcdn.net/jpg/03/13/35/66/500_F_313356641_Gip3M0WffUS3USiUJ1iZxELnp5iNU6pb.jpg"> @EVENTOS: {1}</div>',
            args = { fal, message }
        })
        end
    end, false)
    ...
    RegisterCommand('limpachat', function(source)
        local user_id = vRP.getUserId(source);
        if user_id ~= nil then
            if vRP.hasPermission(user_id, "admin.permissao") or vRP.hasPermission(user_id, "moderador.permissao") or vRP.hasPermission(user_id, "wl.permissao") then
