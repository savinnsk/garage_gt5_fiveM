...
Citizen.CreateThread(function()
	...
	while true do 
		...
		elseif not nui then
			idle = 5
			DrawMarker(27, bennys[1],bennys[2],bennys[3]-0.97 ,0,0,0,0,0,0,3.0,3.0,1.0,40, 34, 213,200,0,0,0,1)
			DrawMarker(36,bennys[1],bennys[2],bennys[3]-0.50,0,0,0,0.0,0,0,0.8,0.8,0.8,40,34,213,200,0,0,0,1)
			--DrawMarker(36, bennys,0,0,0,0,0,0,1.0,3.0,1.0,255, 102, 0,200,0,0,0,1)
			local playercoords = GetEntityCoords(PlayerPedId())
			local distance = #(playercoords - bennys)
			if distance < 2 then
				drawTxt("PRESSIONE  ~b~E~w~  PARA ACESSAR A ~o~MECÂNICA",4,0.5,0.93,0.50,255,255,255,180)
